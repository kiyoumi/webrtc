 ...
 */
/**
 * 房间类 代表一个Licode房间。它处理连接,本地流和发布流,远程流订阅
 * 典型的空间初始化: var room= Erizo.Room({token:“213 h8012hwduahd - 321 ueiwqewq });
 * 它还处理RoomEvents和StreamEvents。例如:
 * 事件“room-connected”指出,用户已成功连接到房间。
 * 事件“room-disconnected”表明用户已经断开连接。
 * 事件“stream-added”表明,房间里有一个新的流可用。
 * 事件“stream-removed”表明,前一个可用流已经离开了房间。
 */
var Erizo = Erizo || {};
...
Erizo.Room = function (spec) {
    ...
    connectSocket = function (token, callback, error) {
        ...
        // "host":"xxx","secure":true,  reconnect: false, secure: token.secure, 'force new connection': true,
        var transport_str = "http://";
        if( token.secure ){
            transport_str = "https://";
        }
        that.socket = io.connect(transport_str + token.host, {transports: ['websocket'], 'force new connection': true });
        //that.socket = io.connect(transport_str + token.host, {transports: ['websocket'] });
       ...
       // 监听erizo信令消息
        that.socket.on('signaling_message_erizo', function (arg) {
            ...
            if (stream && !stream.failed) {
                console.log("stream.pc-------------->"+JSON.stringify(stream.pc));
    ...
    that.connect = function () {
        var streamList = [],
            ...
            console.log("base64解码后的token =========="+ token);
    ...
    that.subscribe = function (stream, options, callback) {
        ...
        if (stream && !stream.local) {
            ...
            if( options.video != undefined )
                stream.video = options.video;
            if( options.audio != undefined )
                stream.audio = options.audio;
